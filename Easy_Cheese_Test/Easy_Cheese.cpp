	...
	AEGP_WindowType			active_window)
{
	...
	if (!err && active_itemH) {
		...
		if (!err && (AEGP_ItemType_COMP == item_type || AEGP_ItemType_FOOTAGE == item_type)) {
...
static A_Err
PrepareForCheese(AEGP_CompH *current_compPH, AEGP_ItemH *active_itemPH)
{
	A_Err err = A_Err_NONE;
...
static A_Err
GetLayerToBeCheesed(const AEGP_CompH compH, AEGP_LayerH *layerPH)
{
	AEGP_SuiteHandler suites(sP);
...
static A_Err
PlayWithMasks(AEGP_LayerH *layerPH)
{
	A_Err err = A_Err_NONE;
	...
	if (!err && num_masksL) {
		err = suites.MaskSuite4()->AEGP_GetLayerMaskByIndex(*layerPH, 0, &maskH);
		if (!err && maskH) {
...
static A_Err
PlayWithExpressions(AEGP_StreamRefH *stream_refPH)
{
	A_Err err = A_Err_NONE, err2 = A_Err_NONE;
...
static A_Err
GetPositionStreamInfo(AEGP_LayerH *layerPH, AEGP_StreamRefH *position_streamPH, A_long *num_keyframesPL, A_short *value_dimsPS, A_Boolean *variesPB)
{
	A_Err err = A_Err_NONE;
	AEGP_StreamFlags stream_flags = AEGP_StreamFlag_NONE;
	A_FpLong minF = 0, maxF = 0;
	AEGP_SuiteHandler suites(sP);

	err = suites.StreamSuite2()->AEGP_GetNewLayerStream(S_my_id, *layerPH, AEGP_LayerStream_POSITION, position_streamPH);
	if (!err && *position_streamPH) {
		err = suites.StreamSuite2()->AEGP_GetStreamProperties(*position_streamPH, &stream_flags, &minF, &maxF);
	}
...
static A_Err
MessWithMarkers(AEGP_LayerH *layerPH, AEGP_StreamRefH *stream_refPH, A_Time *timePT)
{
	A_Err err = A_Err_NONE, err2 = A_Err_NONE;
	A_long new_indexL = 0;
	...
	ERR(suites.KeyframeSuite3()->AEGP_InsertKeyframe(*stream_refPH, AEGP_LTimeMode_LayerTime, timePT, &new_indexL));
	ERR(suites.KeyframeSuite3()->AEGP_GetNewKeyframeValue(S_my_id, *stream_refPH, new_indexL, &value));
...
static A_Err
GetCheesyWithIt(AEGP_StreamRefH *stream_refPH, A_long *keyframe_indexPL, A_short *value_dimsPS)
{
	A_Err err = A_Err_NONE;
	...
	ERR(suites.KeyframeSuite3()->AEGP_GetKeyframeTemporalEase(*stream_refPH, *keyframe_indexPL, temporal_dimsS - 1, &ease_in, &ease_out));
	if (!err) {
		ease_in.speedF = ease_out.speedF = 0.0;
		ease_in.influenceF = ease_out.influenceF = .33333333;
		err = suites.KeyframeSuite3()->AEGP_SetKeyframeTemporalEase(*stream_refPH, *keyframe_indexPL, temporal_dimsS - 1, &ease_in, &ease_out);
		ERR(suites.KeyframeSuite3()->AEGP_SetKeyframeInterpolation(*stream_refPH, *keyframe_indexPL, AEGP_KeyInterp_BEZIER, AEGP_KeyInterp_BEZIER));
	}
	return err;
}

static A_Err
CommandHook(AEGP_GlobalRefcon plugin_refconPV, AEGP_CommandRefcon refconPV, AEGP_Command command, AEGP_HookPriority hook_priority, A_Boolean already_handledB, A_Boolean *handledPB)
{
	A_Err err = A_Err_NONE;
	AEGP_SuiteHandler suites(sP);
	...
	try {
		if (S_Easy_Cheese_cmd == command) {
			*handledPB = TRUE;
			...
			AEGP_StreamRefH position_streamH = NULL, marker_streamH = NULL;
			...
			ERR(GetPositionStreamInfo(&current_layerH, &position_streamH, &num_keyframesL, &value_dimsS, &variesB));
			...
			if (!err && variesB) {
				...
				if (!err && num_keyframesL) {
					err = suites.StreamSuite2()->AEGP_GetNewLayerStream(S_my_id, current_layerH, AEGP_LayerStream_MARKER, &marker_streamH);
					for (A_long indexL = 0; (!err && indexL < num_keyframesL); indexL++) {
						err = suites.KeyframeSuite3()->AEGP_GetKeyframeTime(position_streamH, indexL, AEGP_LTimeMode_LayerTime, &key_timeT);
						ERR(GetCheesyWithIt(&position_streamH, &indexL, &value_dimsS));
