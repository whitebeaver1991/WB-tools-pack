...
function updateGlobals() {
    ...
    if (ba) ba.value = bgAlpha[0];
    var bav = document.getElementById('bgAlphaValue0');
    if (bav) bav.textContent = bgAlpha[0];
    var bc = document.getElementById('bgColorInput0');
    if (bc) bc.value = '#' + bgColor[0];
    var bct = document.getElementById('bgColorText0');
    if (bct) bct.textContent = '#' + bgColor[0].toUpperCase();
    var gc = document.getElementById('glowColorInput0');
    if (gc) gc.value = '#' + glowColor[0];
    var gct = document.getElementById('glowColorText0');
    if (gct) gct.textContent = '#' + glowColor[0].toUpperCase();
    var pgc = document.getElementById('pageColorInput0');
    if (pgc) pgc.value = '#' + pageColor[0];
    var pgct = document.getElementById('pageColorText0');
    if (pgct) pgct.textContent = '#' + pageColor[0].toUpperCase();
    var gi = document.getElementById('glowIntensitySlider0');
    if (gi) gi.value = glowIntensity[0];
    var giv = document.getElementById('glowIntensityValue0');
    if (giv) giv.textContent = glowIntensity[0];
    var id = document.getElementById('imgDistSlider0');
    if (id) id.value = imgDist[0];
    var idv = document.getElementById('imgDistValue0');
    if (idv) idv.textContent = imgDist[0];
    var td = document.getElementById('textDistSlider0');
    if (td) td.value = textDist[0];
    var tdv = document.getElementById('textDistValue0');
    if (tdv) tdv.textContent = textDist[0];
    var ts = document.getElementById('textSizeSlider0');
    if (ts) ts.value = textSize[0];
    var tsv = document.getElementById('textSizeValue0');
    if (tsv) tsv.textContent = textSize[0] + '%';
    // UI Zoom (settings modal)
    var uzs = document.getElementById('uiZoomSliderSettings');
    if (uzs) uzs.value = uiZoom[curMenu];
    var uzvs = document.getElementById('uiZoomValueSettings');
    if (uzvs) uzvs.textContent = uiZoom[curMenu];
    var zc = document.getElementById('zoomContent');
    if (zc) zc.style.zoom = (uiZoom[curMenu] / 100).toFixed(2);
    // Menu scales per menu type
    for (var mi = 0; mi < 4; mi++) {
        var msEl = document.getElementById('menuScaleSlider' + mi);
        if (msEl) msEl.value = menuScale[mi];
        var msvEl = document.getElementById('menuScaleValue' + mi);
        if (msvEl) msvEl.textContent = menuScale[mi] + '%';
    }
    ...
    if (qs) qs.value = '' + (quickStyle || 0);
    // Background paths
    var bgPaths = ['pieBgPath','quickBgPath','wheelBgPath','infiniteBgPath'];
    for (var bi = 0; bi < 4; bi++) {
        var bp = document.getElementById(bgPaths[bi]);
        if (bp) bp.value = g_bgPath ? (g_bgPath[bi] || '') : '';
    }
  }