...
A_Err UpdateMenuHook(AEGP_GlobalRefcon, AEGP_UpdateMenuRefcon, AEGP_WindowType)
{
    ...
    for (int i = 0; i < 6; i++) suites.CommandSuite1()->AEGP_EnableCommand(S_pi_cmd[i]);
    return A_Err_NONE;
}
...
A_Err EntryPointFunc(struct SPBasicSuite *pica_basicP, A_long, A_long, AEGP_PluginID id, AEGP_GlobalRefcon *gr)
{
    ...
    const char *labels[6] = {"wb tools pack", "Quick Menu", "Wheel Menu", "Save Eff", "Apply Eff", "List Numpad Devices"};
    for (int i = 0; i < 6; i++) {
