...
# Load Visual Studio 2026 x64 environment
...
# Common compiler flags (each flag is a separate array element)
$CL_Common = @(
    "/nologo", "/c", "/W3", "/EHsc", "/MT", "/O2"
    "/DMSWindows", "/DWIN32", "/D_WINDOWS", "/DAE_PROC_AMD64=1", "/D_USRDLL=1"
)

# Include paths (each /I and its path are separate elements, handles spaces properly)
$INC = @()
...
# ================== PiPL 资源生成 (使用 generate_pipl.ps1) ==================
